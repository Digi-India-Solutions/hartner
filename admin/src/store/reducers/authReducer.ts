...
const normalizeTokenPayload = (payload: any) => {
  if (!payload) {
    return null;
  }
  ...
  if (payload.data && typeof payload.data === "object") {
    ...
    if (nested.accessToken || nested.refreshToken || nested.token) {
      return {
        accessToken: nested.accessToken || nested.token || "",
        refreshToken: nested.refreshToken || nested.token || nested.accessToken || "",
      };
    }
  }
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addMatcher(
        ...
        (state, action) => {
          const tokens = normalizeTokenPayload(action.payload);
          if (!tokens) {
            return state;
          }
          return _setTokens(state, tokens);
        }
      )
      .addMatcher(
        isAnyOf(
